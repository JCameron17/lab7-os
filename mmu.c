...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    * 6. Deal with the remaining left over memory (frag).
    *     a. dynamically allocate a new block_t called frag [use malloc]
    *     b. set the frag->pid = 0 
    *     c. set the frag->start = the blk.end + 1
    *     d. set the frag->end = original blk.end before you changed it in #4
    *     e. add the frag to the FREE_LIST based on policy
    */
  int remove;
    if (list_is_in_by_size(freelist, blocksize)){
        remove = list_get_index_of_by_Size(freelist, blocksize);
    } else {
      printf("Error: Memory Allocation %d blocks\n", blocksize);
      return;
    }
  
    block_t* block2 = list_remove_at_index(freelist, remove);
    int end = block2->end;
    block2->pid = pid;
    block2->end = block2->start + blocksize-1;
    list_add_ascending_by_address(alloclist, block2);
  
    block_t* frag = malloc(sizeof(block_t));
    frag->pid = 0;
    frag->start = block2->end + 1;
    frag->end = end;
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
  ...
  block_t* block;
    ...
    if (list_is_in_by_pid(alloclist, pid)){
      ...
      block = list_remove_at_index(alloclist, remove);
    }
    else{
